...
func testEtcdStoragePath(t *testing.T, legacy bool) {
	...
	for gvk, apiType := range kapi.Scheme.AllKnownTypes() {
		...
		if !hasTest && !isEphemeral {
			t.Errorf("no test data for %s from %s.  Please add a test for your new type to etcdStorageData.", gvk, pkgPath)
			continue
		}

		if hasTest && isEphemeral {
			t.Errorf("duplicate test data for %s from %s.  Object has both test data and is ephemeral.", gvk, pkgPath)
			continue
		}
