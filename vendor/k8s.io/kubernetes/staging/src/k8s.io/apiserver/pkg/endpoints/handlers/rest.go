	...
	applyPatch := func(_ request.Context, _, currentObject runtime.Object) (runtime.Object, error) {
		...
		case originalObj == nil && originalObjJS == nil && originalObjMap == nil:
			...
			case types.StrategicMergePatchType:
				...
				patchMap, err := strategicPatchObjectInPlace(codec, temporaryOriginalObjMap, patchJS, versionedObjToUpdate, versionedObj)
				if err != nil {
					return nil, err
				}
		...
		default:
			...
			if err := applyPatchToObjectInPlace(codec, currentObjMap, originalPatchMap, versionedObjToUpdate, versionedObj); err != nil {
				return nil, err
			}
			// Convert the object back to unversioned.
			gvk := kind.GroupKind().WithVersion(runtime.APIVersionInternal)
