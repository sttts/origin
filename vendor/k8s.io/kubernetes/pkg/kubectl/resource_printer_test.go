...
func TestPrinter(t *testing.T) {
	...
	for _, test := range printerTests {
		...
		printer, found, err := GetPrinter(test.Format, test.FormatArgument, false, false)
		if err != nil || !found {
...
func TestAllowMissingKeys(t *testing.T) {
	tests := []struct {
		...
		{"test template, allow missing keys", false, "template", "{{.blarg}}", &api.Pod{}, "<no value>", ""},
		{"test template, strict", true, "template", "{{.blarg}}", &api.Pod{}, "", `error executing template "{{.blarg}}": template: output:1:2: executing "output" at <.blarg>: map has no entry for key "blarg"`},
		{"test jsonpath, allow missing keys", false, "jsonpath", "{.blarg}", &api.Pod{}, "", ""},
		{"test jsonpath, strict", true, "jsonpath", "{.blarg}", &api.Pod{}, "", "error executing jsonpath \"{.blarg}\": blarg is not found\n"},
	}
	for _, test := range tests {
		buf := bytes.NewBuffer([]byte{})
		printer, _, err := GetPrinter(test.Format, test.Template, false, test.Strict)
		if err != nil {
			t.Errorf("in %s, unexpected error: %#v", test.Name, err)
		}
