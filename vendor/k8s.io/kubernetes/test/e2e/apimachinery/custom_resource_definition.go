...
var crdPublishOpenAPIVersion = utilversion.MustParseSemantic("v1.12.0")
	...
	Context("Simple CustomResourceDefinition", func() {
		...
		*/
		...
		It("has OpenAPI spec served with CRD Validation schema", func() {
			...
			if testcrd.ApiGroup != testcrd2.ApiGroup {
			...
			crdDefinitionKey := openapi.FriendlyName(fmt.Sprintf("%s/%s/%s", testcrd.ApiGroup, testcrd.Name, testcrd.Kind))
			crd2DefinitionKey := openapi.FriendlyName(fmt.Sprintf("%s/%s/%s", testcrd2.ApiGroup, testcrd2.Name, testcrd2.Kind))
			...
			input := fmt.Sprintf("{\"kind\":\"%s\",\"apiVersion\":\"%s/%s\",\"metadata\":{\"name\":\"foo\"},\"spec\":{\"foo\":true}}", testcrd.Kind, testcrd.ApiGroup, testcrd.Name)
			...
			input = fmt.Sprintf("{\"kind\":\"%s\",\"apiVersion\":\"%s/%s\",\"metadata\":{\"name\":\"foo\"},\"spec\":{\"bars\":[{\"name\":\"test\"}]}}", testcrd.Kind, testcrd.ApiGroup, testcrd.Name)
